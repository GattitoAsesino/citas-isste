...
public class Formato_Citas {
    ...
    public static List<String> obtenerHorasDisponiblesLista(Connection conn, String fecha, int idMedico) {
...
public static List<String> obtenerHorasReservadasLista(Connection conn, String fecha, int idMedico) {
    List<String> horas = new ArrayList<>();
    try {
        String query = "SELECT DISTINCT hora FROM Citas WHERE fecha = ? AND id_medico = ? AND disponible = FALSE";
        PreparedStatement stmt = conn.prepareStatement(query);
        stmt.setString(1, fecha);
        stmt.setInt(2, idMedico);
        ResultSet rs = stmt.executeQuery();
