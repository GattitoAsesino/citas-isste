...
import Interpreter.DiagnosticoExpression;
import Interpreter.Expression;
import Interpreter.TratamientoExpression;
...
public class Registrar_Informe {
    ...
    // Crear validadores
    Expression validadorDiagnostico = new DiagnosticoExpression();
    Expression validadorTratamiento = new TratamientoExpression();
    
    // Validar
    if(!validadorDiagnostico.interpret(diagnostico)) {
        JOptionPane.showMessageDialog(null, "Diagnóstico no válido", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }
    
    if(!validadorTratamiento.interpret(tratamiento)) {
        JOptionPane.showMessageDialog(null, "Tratamiento no válido", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }
