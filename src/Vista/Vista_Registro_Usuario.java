...
import Vista.CamposVaciosHandler;
import Vista.CurpValidationHandler;
import Vista.RfcValidationHandler;
import Vista.PasswordValidationHandler;
...
public class Vista_Registro_Usuario extends javax.swing.JFrame {
  ...
  private Subject registroSubject; 
    /**
    ...
    public Vista_Registro_Usuario(Connection Cx) {
        initComponents();
        conn = Cx;
    registroSubject = new RegistroSubject(); // Inicializa el Subject
        registroSubject.addObserver(new EmailBienvenidaObserver()); // Registra el observer
    }
    
  private boolean validarFormulario() {
    // Verificar que las contraseñas coincidan
    if (!new String(contraseña.getPassword()).equals(new String(confirmar_contraseña.getPassword()))) {
    ...
    Usuario usuario = new Usuario(
        ...
        nombre.getText().trim(), 
        apellido_m.getText().trim(), 
        apellido_p.getText().trim()
    );

    // Configurar cadena de validación
    ValidacionHandler handlerChain = new CamposVaciosHandler();
    handlerChain.setNextHandler(new CurpValidationHandler())
               .setNextHandler(new RfcValidationHandler())
               .setNextHandler(new PasswordValidationHandler());

    // Ejecutar validaciones
    return handlerChain.validar(usuario);
}
    /**
    ...
    private void RegistrarseMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_RegistrarseMouseClicked
        ...
        try {
            if (RegistrarUsuario.registrar(conn, usuario)) {
                ...
                ((RegistroSubject)registroSubject).notifyRegistroExitoso(usuario);
                
                // Redirigir a login
                new Iniciar_Sesion(conn).setVisible(true);
                this.dispose();
            }
